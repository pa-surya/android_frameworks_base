...
public class NetworkTraffic extends TextView implements TunerService.Tunable,
    ...
    protected void onAttachedToWindow() {
        ...
        if (mNetdService == null) {
            Log.w(TAG, "NetdService is NULL! Tethering stats will not be read.");
        }
    ...
    private void updateNetworkTraffic(boolean initial) {
        ...
        long tetheringBytes = 0;
        if (mNetdService != null) {
            try {
                tetheringBytes = Arrays.stream(mNetdService.tetherGetStats())
                                    .mapToLong(t -> (t.rxBytes + t.txBytes))
                                    .sum();
            } catch (Exception e) {
                Log.w(TAG, "Error while reading tethering stats:", e);
            }
        }

        dlog("updateNetworkTraffic totalBytes=" + totalBytes + " tetheringBytes=" + tetheringBytes
                + " mLastTotalBytes=" + mLastTotalBytes + " initial=" + initial);

        totalBytes += tetheringBytes;
    ...
    private boolean shouldBeVisible() {
        ...
        dlog("shouldBeVisible mHide=" + mHide + " mScreenOff=" + mScreenOff
                    + " connected=" + connected);
        return !mHide && !mScreenOff && connected;
    }
